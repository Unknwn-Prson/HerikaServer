...
/**
 * Extracts speaker name from the last user message in context
 */
function getLastUserMessageSpeaker($contextData) {
    ...
    // Fallback to player name global
    return isset($GLOBALS["PLAYER_NAME"]) ? $GLOBALS["PLAYER_NAME"] : 'Player';
}

/**
 * Builds simple format instruction based on enabled features
 * Order: (mood)(listener)(action)(target)
 */
function buildSimpleFormatInstruction($includeMood, $includeListener, $includeActions, $includeTarget, $customInstruction = '') {
    ...
    // DON'T include customInstruction here - it has action instructions
...
/**
 * Extracts simple format from buffer: (mood)(listener)(action)(target) message
 * Order: mood, listener, action, target
 */
...
/**
 * Validates action name against common actions, returns default if invalid
 */
function validateActionName($action) {
    ...
    // Invalid action, log and return default
...
class openrouterjsonanthropic
{
    ...
    // New properties for response format configuration
    ...
    public function open($contextData, $customParms)
    {
        ...
        $response_format = isset($GLOBALS["CONNECTOR"][$this->name]["response_format"]) 
            && in_array($GLOBALS["CONNECTOR"][$this->name]["response_format"], ['json', 'simple'])
            ? $GLOBALS["CONNECTOR"][$this->name]["response_format"] 
            : 'json';

        $include_actions_list = isset($GLOBALS["CONNECTOR"][$this->name]["include_actions_list"]) 
            ? (bool)$GLOBALS["CONNECTOR"][$this->name]["include_actions_list"] 
            : true;

        $include_mood_requirement = isset($GLOBALS["CONNECTOR"][$this->name]["include_mood_requirement"]) 
            ? (bool)$GLOBALS["CONNECTOR"][$this->name]["include_mood_requirement"] 
            : true;

        $include_target_requirement = isset($GLOBALS["CONNECTOR"][$this->name]["include_target_requirement"]) 
            ? (bool)$GLOBALS["CONNECTOR"][$this->name]["include_target_requirement"] 
            : true;

        $include_listener_requirement = isset($GLOBALS["CONNECTOR"][$this->name]["include_listener_requirement"]) 
            ? (bool)$GLOBALS["CONNECTOR"][$this->name]["include_listener_requirement"] 
            : true;

        // Enforce dependency: target required if actions enabled
        if ($include_actions_list) {
            $include_target_requirement = true;
        }

        logMessage("Response Format Config: format=$response_format, actions=$include_actions_list, mood=$include_mood_requirement, target=$include_target_requirement, listener=$include_listener_requirement, uncached=$dialogue_cache_uncached_count");
        ...
        // Build actions and response format instruction based on config
        ...
        if ($response_format === 'json') {
            // JSON format - create local copy and modify based on disabled features
            $template = $GLOBALS["responseTemplate"];
            
            if (!$include_mood_requirement && is_array($template) && isset($template['mood'])) {
                unset($template['mood']);
            }
            
            if (!$include_actions_list && is_array($template) && isset($template['action'])) {
                unset($template['action']);
            }
            
            if (!$include_target_requirement && is_array($template) && isset($template['target'])) {
                unset($template['target']);
            }
            
            if (!$include_listener_requirement && is_array($template) && isset($template['listener'])) {
        ...
        // Combine actions and format instruction
        $actionsText = "";
        ...
        // Add prefill for simple format with Anthropic to enforce format
        if ($response_format === 'simple' && $provider_caching === "Anthropic") {
            // Add user message with dialogue history
            $finalMessagesToSend[] = array('role' => 'user', 'content' => $completeEventList);
            // Add assistant prefill to force format compliance
            $prefillText = '(';
            $finalMessagesToSend[] = array('role' => 'assistant', 'content' => array(
                array('type' => 'text', 'text' => $prefillText)
            ));
            
            // Track that we used prefill
            $this->_usedPrefill = true;
            $this->_prefillContent = $prefillText;
        } else {
            // JSON format or non-Anthropic provider
            $finalMessagesToSend[] = array('role' => 'user', 'content' => $completeEventList);
            $this->_usedPrefill = false;
            $this->_prefillContent = '';
        }

        // Calculate cache control index based on dialogue_cache_uncached_count
        ...
        // Store config in instance variables for use in process() and processActions()
        $this->_responseFormat = $response_format;
        $this->_includeMood = $include_mood_requirement;
        $this->_includeActions = $include_actions_list;
        $this->_includeTarget = $include_target_requirement;
        $this->_includeListener = $include_listener_requirement;
        ...
        if (!empty($buffer)) {
            if ($this->_responseFormat === 'json') {
                ...
                if (json_last_error() === JSON_ERROR_NONE && isset($tempJson['message']) && !empty($tempJson['message'])) {
                    // Always set animation/expression, let functions handle empty mood
                    $mood = isset($tempJson["mood"]) ? $tempJson["mood"] : '';
                    $GLOBALS["SCRIPTLINE_ANIMATION"] = GetAnimationHex($mood);
                    $GLOBALS["SCRIPTLINE_EXPRESSION"] = GetExpression($mood);
                ...
                // If we used prefill, prepend it to buffer for parsing
                $bufferToProcess = $this->_usedPrefill ? ($this->_prefillContent . $this->_buffer) : $this->_buffer;
                ...
                if ($parsed['found']) {
                    if (!$this->_simpleFormatParsed) {
                        ...
                        $GLOBALS["SCRIPTLINE_ANIMATION"] = GetAnimationHex($parsed['mood']);
                        $GLOBALS["SCRIPTLINE_EXPRESSION"] = GetExpression($parsed['mood']);
                        
                        if ($this->_includeListener && !empty($parsed['listener'])) {
                            $GLOBALS["SCRIPTLINE_LISTENER"] = $parsed['listener'];
                        } else {
                            $GLOBALS["SCRIPTLINE_LISTENER"] = $this->_defaultTarget;
                        }
                    }
                    
                    // Return only NEW content (streaming)
                    $currentMessage = $parsed['message'];
                    
                    if (!isset($this->_lastReturnedLength)) {
                        $this->_lastReturnedLength = 0;
                    }
                    
                    // Extract only the new chunk
                    if (strlen($currentMessage) > $this->_lastReturnedLength) {
                        $newContent = substr($currentMessage, $this->_lastReturnedLength);
                        $this->_lastReturnedLength = strlen($currentMessage);
                        return $newContent;
                    }
                }
                // If parsing fails or no new content, return empty
                return "";
            }
        }
    ...
    public function close()
    {
        ...
        $this->_parameterBuff = "";
        $this->_forcedClose = false;
        $this->_simpleFormatParsed = false; // Reset parse flag for next request
    ...
    public function processActions()
    {
        ...
        // Parse buffer based on response format
        if (!empty($this->_buffer)) {
            ...
            if ($this->_responseFormat === 'json') {
                ...
                if ($jsonStart !== false && $jsonEnd !== false && $jsonEnd > $jsonStart) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE && is_array($parsedResponse)) {
                        ...
                        if (isset($parsedResponse['character']) && !empty($parsedResponse['character'])) {
            ...
            // Generate command if we have an action
            if (!empty($action)) {
                ...
                if (!isset($alreadysent[$commandKey]) || empty($alreadysent[$commandKey])) {
                    ...
                    logMessage("[{$this->name}:{$herikaName}] Command already sent (skipped): {$character}|command|{$action}@{$target}");
                }
            } else {
                logMessage("[{$this->name}:{$herikaName}] No action extracted from buffer.");
            }
        } else {
            logMessage("[{$this->name}:{$herikaName}] Buffer is empty, no actions to process.");
        }
